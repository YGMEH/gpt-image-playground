...
export interface ApiProfile {
  ...
  providerDrafts?: Partial<Record<ApiProvider, Partial<Pick<ApiProfile, 'baseUrl' | 'model' | 'availableModels' | 'apiMode' | 'reasoningEffort' | 'codexCli' | 'apiProxy' | 'responseFormatB64Json' | 'streamImages' | 'streamPartialImages'>>>>
}
...
export interface TaskRecord {
  ...
  /** 生成时使用的模型 ID */
  apiModel?: string
  /** 生成时使用的 API 地址（不含密钥），用于任务溯源和服务商专属恢复入口 */
  apiBaseUrl?: string
