...
export function createPersistedState(state: PersistedStateSource, includeLegacyAgentConversations = false): PersistedAppState {
  const settings = ensureLocalStudioSettings(normalizeSettings(state.settings))
...
export function normalizePersistedState(
  ...
  const settings = ensureLocalStudioSettings(normalizeSettings(persistedState.settings ?? fallback.settings))
