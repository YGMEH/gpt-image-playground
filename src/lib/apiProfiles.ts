...
import { allowsEmptyDeepSeekApiKey, DEFAULT_CHAT_MODEL } from './deepseekModelCatalog'
...
export function normalizeAvailableModels(input: unknown): string[] | undefined {
  if (!Array.isArray(input)) return undefined
  const models = [...new Set(input.map((item) => typeof item === 'string' ? item.trim() : '').filter(Boolean))].sort()
...
export function isChatCompletionsApiProfile(profile: ApiProfile): boolean {
  return profile.provider === 'openai' && profile.apiMode === 'chat'
}
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  ...
  if (provider === 'fal') {
    return {
      ...
      availableModels: savedDraft?.availableModels,
      apiMode: 'images',
...
function normalizeProviderDraft(input: unknown, provider: ApiProvider, customProviderIds: Set<string>): ApiProfileProviderDraft {
  ...
  const apiMode = input.apiMode === 'responses' || input.apiMode === 'chat' || input.apiMode === 'images' ? input.apiMode : undefined
  ...
  return {
    baseUrl: provider === 'fal'
      ...
      : baseUrl,
    model,
    availableModels: normalizeAvailableModels(input.availableModels),
    apiMode,
...
export function normalizeApiProfile(input: unknown, fallback?: Partial<ApiProfile>, customProviderIds = new Set<string>()): ApiProfile {
  ...
  const apiMode: ApiMode = provider === 'openai' && (record.apiMode === 'responses' || record.apiMode === 'chat') ? record.apiMode : 'images'
  ...
  return {
    ...
    availableModels: normalizeAvailableModels(record.availableModels) ?? defaults.availableModels,
...
export function getActiveApiProfile(settings: Partial<AppSettings> | unknown): ApiProfile {
  ...
  const apiMode = profile.provider === 'openai' && (record.apiMode === 'images' || record.apiMode === 'responses' || record.apiMode === 'chat')
    ? record.apiMode
    : profile.apiMode
...
export function validateApiProfile(profile: ApiProfile): string | null {
  ...
  if (!profile.apiKey.trim() && !allowsEmptyDeepSeekApiKey(profile)) return '缺少 API Key'
