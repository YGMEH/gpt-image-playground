...
describe('callImageApi', () => {
  ...
  it('keeps async submit responses small and accepts a nested task id fallback', async () => {
    const fetchMock = vi.spyOn(globalThis, 'fetch')
      .mockResolvedValueOnce(new Response(JSON.stringify({ data: { id: 'task-nested-1' } }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }))
      .mockResolvedValueOnce(new Response(JSON.stringify({
        status: 'succeeded',
        results: [{ b64_json: 'aW1hZ2U=' }],
      }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }))
    ...
    expect(onCustomTaskEnqueued).toHaveBeenCalledWith({ taskId: 'task-nested-1' })
    const firstRequest = fetchMock.mock.calls[0]
    expect(firstRequest?.[0]).toBe('https://api.example.com/v1/images/generations')
    const submitBody = JSON.parse(String(firstRequest?.[1]?.body))
    expect(submitBody).toMatchObject({ prompt: 'prompt', replyType: 'async' })
    expect(submitBody).not.toHaveProperty('response_format')
  })
