...
function isLikelyCustomTaskId(value: string): boolean {
  const trimmed = value.trim()
  if (!trimmed || trimmed.length > 200) return false
  if (/^https?:\/\//i.test(trimmed) || trimmed.startsWith('data:')) return false
  return /^[A-Za-z0-9][A-Za-z0-9._:-]{5,}$/.test(trimmed)
}

function extractCustomTaskId(payload: unknown, taskIdPath?: string): string {
  const candidates: unknown[] = []
  if (taskIdPath) {
    candidates.push(getByPath(payload, taskIdPath))
    if (taskIdPath === 'id') candidates.push(getByPath(payload, 'data.id'))
    if (taskIdPath === 'data.id') candidates.push(getByPath(payload, 'id'))
  }
  candidates.push(
    ...
    getByPath(payload, 'data.taskId'),
  )

  for (const candidate of candidates) {
    if (typeof candidate === 'string' || typeof candidate === 'number') {
      const taskId = String(candidate).trim()
      if (isLikelyCustomTaskId(taskId)) return taskId
    }
  }
  return ''
}
...
async function submitCustomRequest(mapping: CustomProviderSubmitMapping, opts: CallApiOptions, profile: ApiProfile, controller: AbortController, proxyConfig: ReturnType<typeof readClientDevProxyConfig>, useApiProxy: boolean): Promise<unknown> {
  ...
  if (method !== 'GET') {
    if (contentType === 'multipart') {
      ...
      if (!mapping.taskIdPath && profile.responseFormatB64Json) {
        formData.append('response_format', 'b64_json')
      }
      body = formData
    } else {
      ...
      if (resolved && typeof resolved === 'object' && !Array.isArray(resolved)) {
        const bodyRecord = resolved as Record<string, unknown>
        if (mapping.taskIdPath) {
          if (bodyRecord.replyType == null && bodyRecord.reply_type == null) {
            bodyRecord.replyType = 'async'
          }
        } else if (profile.responseFormatB64Json) {
          bodyRecord.response_format = 'b64_json'
        }
      }
...
async function callCustomHttpImageApi(opts: CallApiOptions, profile: ApiProfile, customProvider: CustomProviderDefinition): Promise<CallApiResult> {
  ...
  try {
    ...
    const taskId = submitMapping.taskIdPath ? extractCustomTaskId(submitPayload, submitMapping.taskIdPath) : ''
    if (submitMapping.taskIdPath && !taskId) {
      try {
        return await extractCustomImages(submitPayload, submitMapping.result ?? {}, mime, controller.signal)
      } catch {
    ...
    try {
      ...
      console.warn('Failed to persist custom task id', err)
    }
    if (timeoutId) {
      clearTimeout(timeoutId)
      timeoutId = null
