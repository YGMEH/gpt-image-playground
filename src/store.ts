...
import { DEFAULT_SETTINGS, getActiveApiProfile, getAgentImageApiProfile, getAgentTextApiProfile, getCustomProviderDefinition, isAgentTextApiProfile, mergeImportedSettings, normalizeSettings, validateApiProfile } from './lib/apiProfiles'
import { ensureLocalStudioSettings } from './lib/localStudioProfiles'
...
import { prepareUploadImage, type PreparedUploadImage } from './lib/uploadImagePreprocess'
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      setAppMode: (appMode) => {
        ...
        if (settings.agentApiConfigMode === 'off' && !isAgentTextApiProfile(activeProfile)) {
          state.setConfirmDialog({
            ...
            cancelText: '取消',
            action: () => {
              useStore.getState().setShowSettings(true, 'text')
            },
          })
          return
        }
        ...
        if (settings.agentApiConfigMode !== 'off') {
          state.setConfirmDialog({
            ...
            message: `${agentValidationError.message}\n\n请前往「文本模型」或 Agent 配置页，选择或新建可用配置。`,
            confirmText: '去设置',
            cancelText: '取消',
            action: () => {
              useStore.getState().setShowSettings(true, agentValidationError.message.includes('图像模型') ? 'agent' : 'text')
            },
          })
          return
        }
      ...
      // Settings
      settings: ensureLocalStudioSettings({ ...DEFAULT_SETTINGS }),
...
function isRunningOpenAITask(task: TaskRecord) {
  return task.status === 'running' && (task.apiProvider ?? 'openai') !== 'fal' && !task.customTaskId
}
...
function getAgentProfileValidationError(settings: AppSettings): { profile: ApiProfile | null; message: string } | null {
  ...
  if (!textProfile || !isAgentTextApiProfile(textProfile)) {
    return { profile: textProfile, message: 'Agent 模式需要使用 Chat Completions 或 Responses API 的 OpenAI 兼容文本模型配置。' }
  }
...
export async function initStore() {
  ...
  for (const task of tasks) {
    ...
    if (
      task.customTaskId &&
      (task.status === 'running' || task.customRecoverable || (task.status === 'error' && !task.outputImages.length))
    ) {
...
export async function submitAgentMessage() {
  ...
  if (agentValidationError) {
    showToast(`请先完善 Agent API 配置：${agentValidationError.message}`, 'error')
    state.setShowSettings(true, agentValidationError.message.includes('图像模型') ? 'agent' : 'text')
    return
  }
...
export function updateTaskInStore(taskId: string, patch: Partial<TaskRecord>) {
  ...
  return task ? putTask(task) : Promise.resolve()
}
...
export async function clearData(options: ClearOptions = { clearConfig: true, clearTasks: true }) {
  ...
  if (options.clearConfig) {
    ...
    setSettings(ensureLocalStudioSettings({ ...DEFAULT_SETTINGS }))
...
function canRecoverCustomTask(task: TaskRecord | undefined): task is TaskRecord {
  if (!task || !task.customTaskId || task.status === 'done' || task.error === AGENT_STOPPED_MESSAGE) return false
  return task.status === 'running' || task.customRecoverable || (task.status === 'error' && !task.outputImages.length)
}

async function completeRecoveredCustomTask(task: TaskRecord, result: Awaited<ReturnType<typeof getCustomQueuedImageResult>>) {
  const latest = useStore.getState().tasks.find((item) => item.id === task.id)
  if (!canRecoverCustomTask(latest)) return

  const { outputIds, outputDataUrls, outputImageSizes, transparentOriginalImageIds } = await storeTaskOutputImages(task, result.images)
  const actualParamsList = await resolveImageSizeParamsList(outputDataUrls, undefined, outputImageSizes)
  const latestBeforeUpdate = useStore.getState().tasks.find((item) => item.id === task.id)
  if (!canRecoverCustomTask(latestBeforeUpdate)) {
    await deleteUnreferencedImageIds([...outputIds, ...(transparentOriginalImageIds ?? [])])
    return
  }
...
async function recoverCustomTask(taskId: string) {
  const { settings, tasks } = useStore.getState()
  const task = tasks.find((item) => item.id === taskId)
  if (!canRecoverCustomTask(task)) return
  ...
  try {
    const result = await getCustomQueuedImageResult(profile, customProvider, customTaskId, task.params)
...
/** 添加图片到输入（文件上传），并返回是否发生了兼容格式转换。 */
export async function addImageFromFile(file: File): Promise<PreparedUploadImage> {
  const prepared = await prepareUploadImage(file)
  const image = await createInputImageFromPreparedUpload(prepared)
  useStore.getState().addInputImage(image)
  return prepared
}

export async function createInputImageFromFile(file: File): Promise<InputImage> {
  return createInputImageFromPreparedUpload(await prepareUploadImage(file))
}

async function createInputImageFromPreparedUpload(prepared: PreparedUploadImage): Promise<InputImage> {
  const id = await storeImage(prepared.dataUrl, 'upload')
