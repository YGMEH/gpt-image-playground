...
export default function SettingsModal() {
  ...
  const canRefreshActiveModels = canRefreshProviderModels(draft, activeProfile)
  const activeAvailableModels = activeProfile.availableModels ?? []
  const modelSelectOptions = [
    ...activeAvailableModels.map((model) => ({ label: model, value: model })),
    ...(activeProfile.model.trim() && !activeAvailableModels.includes(activeProfile.model.trim())
      ? [{ label: `${activeProfile.model.trim()}（当前）`, value: activeProfile.model.trim() }]
      : []),
  ]
  ...
  useEffect(() => {
    ...
    if (activeAvailableModels.length) {
      setModelRefreshStatus(`已缓存 ${activeAvailableModels.length} 个模型`)
      return
    }
    if (!canRefreshActiveModels) {
      setModelRefreshStatus('当前服务商不支持自动刷新模型')
      return
    }
    setModelRefreshStatus('尚未刷新')
  }, [activeProfile.id, activeProfile.provider, activeAvailableModels.length, canRefreshActiveModels, activeCustomProviderAsync])
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="w-full sm:w-48 shrink-0 flex flex-col border-b sm:border-b-0 sm:border-r border-gray-100 dark:border-white/[0.08] bg-gray-50/50 dark:bg-white/[0.02]">
            <nav className="flex-1 overflow-x-auto sm:overflow-y-auto custom-scrollbar p-3 space-x-1 sm:space-x-0 sm:space-y-1 flex sm:flex-col">
              ...
              <button
                ...
                className={`whitespace-nowrap flex-shrink-0 flex items-center gap-2.5 px-3 py-2.5 text-sm rounded-xl transition-colors ${activeTab === 'text' ? 'bg-white dark:bg-white/[0.08] shadow-sm text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100/80 dark:hover:bg-white/[0.04]'}`}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h8M8 14h5M7 4h10a2 2 0 012 2v14l-4-2-3 2-3-2-4 2V6a2 2 0 012-2z" />
                </svg>
                文本模型
              </button>
              <button
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'text' && (
              <TextSettingsTab
                draft={draft}
                selectedTextProfile={selectedAgentTextProfile}
                textProfiles={agentTextProfiles}
                              ...
                              >
                                ...
                                <div className="flex shrink-0 items-center gap-1">
                                  <button
                                    ...
                                    className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-gray-400 opacity-60 transition-[width,opacity] hover:bg-gray-100 hover:text-gray-600 hover:opacity-100 dark:hover:bg-white/[0.08] dark:hover:text-gray-200"
                                  ...
                                  {draft.profiles.length > 1 && (
                                    <button
                                      ...
                                      className="flex h-5 w-5 shrink-0 items-center justify-center rounded text-gray-400 opacity-60 transition-[width,opacity] hover:bg-red-50 hover:text-red-500 hover:opacity-100 dark:hover:bg-red-500/10"
                                      aria-label="删除配置"
                                    >
                ...
                */}
                ...
                <div className="flex flex-wrap items-center justify-center gap-3">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-gray-100/80 px-5 py-2.5 text-sm font-medium text-gray-700 transition-[color,background-color,border-color,opacity,box-shadow,transform] hover:bg-gray-200 hover:text-gray-900 dark:bg-white/[0.06] dark:text-gray-300 dark:hover:bg-white/[0.1] dark:hover:text-white"
                  >
